...
import { CovidContext } from "./contexts/context";

const PlotJapan = () => {

  const {covidNum} = useContext(CovidContext);
  const ref = useRef();
  ...
  useEffect(() => {
    ...
    if(covidNum.length>0 ){
      geoJson.features.forEach(item => {
        ...
        }else if(typeof obj !== "undefined") Object.defineProperty(item.properties, "npatients", {value: obj.npatients,writable: true});
        //item.properties["npatients"] = obj.npatients;
      });
    }
